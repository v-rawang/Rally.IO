...
namespace Rally.Lib.Utility.Math
{
    public class MathUtility
    {
        ...
        public static double[] Compute(double[] InputX, double A, double B, double C)
        {
            ...
            for (int i = 0; i < InputX.Length; i++)
            {
                ...
                result[i] = InputX[i] * InputX[i] * A + InputX[i] * B + C;
            }
        ...
        public static double[] CenterOfArea(double[] InputData, int StartIndex, int EndIndex, int Distance)
        {
            ...
            {
                resultData = new double[InputData.Length];
        ...
        /// 寻峰函数
        /// </summary>
        /// <param name="n">待分析元素、道址总数（如，1024）</param>
        /// <param name="m">窗宽</param>
        /// <param name="H">半高宽</param>
        /// <param name="argA">待分析数组</param>
        /// <param name="argB">寻峰结果数组</param>
        /// <param name="R">预先给定常数（寻峰阈值R）</param>
        /// <param name="channelCount">道址数</param>
        public static void PeakSearch(int n, int m, int H, double[] argA, ref List<int> argB, double R, int channelCount = 1024)
        {
            double a = 0;
            int num = 0;
            bool first = true;
            ...
            {
                if (IsPossiblePeak(n, i, m, H, argA) > R)
                {
                    if (first == true)
                    {
                        first = false;
            ...
            if (argB != null)
            {
                argB = argB.Distinct().ToList();

                for (int i = 0; i < argB.Count; i++)
                {
                    if ((argB[i] != (argA.Length -1)) && (argA[argB[i] + 1] > argA[argB[i]]))
                    {
                        argB[i]++;
                        i--;
                    }
        ...
        public static double ComputeCredibility(double[] E0, double[] E1, double ETOL)
        {
            ...
            double h = 1 + 1.0 * (n / T - 1);/*1 + 1.8 * (n / T - 1);*/
        ...
        public static double PeakIF(IDictionary<int, double> data)
        {
            double variance = -1;
            int rangeLength = -1;
